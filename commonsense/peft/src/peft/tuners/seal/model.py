...
from .layer import Conv2d, SealLayer, dispatch_default
...
class SealModel(BaseTuner):
    ...
    Example:
        ...
        >>> import torch
        >>> import transformers
        >>> from peft import SealConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training
    ...
    **Attributes**:
    ...
    def _check_target_module_exists(seal_config, key):
        return check_target_module_exists(seal_config, key)

    def _prepare_model(self, peft_config: SealConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        pattern_keys = list(chain(seal_config.rank_pattern.keys(), seal_config.alpha_pattern.keys()))
        target_name_key = next(filter(lambda key: re.match(rf".*\.{key}$", current_key), pattern_keys), current_key)
        r = seal_config.rank_pattern.get(target_name_key, seal_config.r)
        alpha = seal_config.alpha_pattern.get(target_name_key, seal_config.lora_alpha)
        ...
        kwargs = {
            ...
            "use_rslora": seal_config.use_rslora,
            "use_dora": seal_config.use_dora,
    ...
    @staticmethod
    def _create_new_module(seal_config, adapter_name, target, **kwargs):
        ...
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, seal_config=seal_config, **kwargs)
